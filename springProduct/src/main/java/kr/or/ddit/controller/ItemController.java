...
public class ItemController {
	...
	public String uploadFormAction(MultipartFile[] uploadFile,Model model) {
		...
		/* 
		 ...
		 --잘 안쓰는 방법
		 String.. = file.getName()
		 byte[].. = file.getBytes() : byte[]로 파일 데이터로 변환
		...
		for(MultipartFile multipartFile : uploadFile) {
			...
			//------------------같은날 같은 이미지를 업로드 시 파일 중복 방지 시작------------------
			//File 객체 설계(복사할 대상 서버 경로(서버쪽), 파일명(uuid가 붙은))
			//uploadFolder : C:\\eclipse-jee-2020-06-R-win32-x86_64\\workspace\\springProduct\\src\\main\\webapp\\resources\\upload\\2022\\10\\28
