...
public class BoardController {
	/* RequestMapping의 value속성에 요청 경로를 설정
	 - 요청 경로는 반드시 설정해햐 하는 필수 정보/ 생략불가능
	...
	/* 2. 경로 패턴 매핑
	 요청 경로를 동적으 표현이 가능한 경로 패텅르 지정할 수 있음
...
//		return mav;
	...
	/* 3.Http 메서드 매핑
	 method 속성을 사용하여 HTTP메서드를 매핑 조건으로 지정가능
	 화면으로 응답하는 경우HTTP메서드로 GET방식과 POST방식 두 가지를 사용함
	 */
	...
	// /7 => 경로 변수
	//골뱅이ReponseBody를 통해서 JSON데이터로 리턴
	@RequestMapping(value="/{boardNo}",method = RequestMethod.PUT,headers=
